...
export default class Stats extends CommandContext {
	...
	public async run(message: Message): Promise<void> {
		...
		const embed = new EmbedBuilder()
            ...
            .addFields({ name: "Users", value: `${this.client.guilds.cache.reduce((a, g) => a + g.memberCount, 0)}`, inline: true })
            .addFields({ name: "Host Type", value: `${os.type()}`, inline: true })
            .addFields({ name: "Load Avg", value: `${os.loadavg().join(", ")}`, inline: true })
            .addFields({ name: "Free Mem", value: `${this.bytesToSize(os.totalmem() - os.freemem())} / ${this.bytesToSize(os.totalmem())}`, inline: true })
